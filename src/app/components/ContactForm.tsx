...
const ContacthtmlForm = () => {
  ...
  // Shows alert message htmlFor htmlForm submission feedback
  ...
  //   Function called on submit that uses emailjs to send email of valid contact htmlForm
  const onSubmit = async (data: any) => {
    // Destrcture data object
    const { name, email, message } = data;
    try {
      //   Disable htmlForm while processing submission
      setDisabled(true);
      ...
      toggleAlert('htmlForm submission was successful!', 'success');
    } catch (e) {
      console.error(e);
      // Display error alert
      toggleAlert('Uh oh. Something went wrong.', 'danger');
    } finally {
      // Re-enable htmlForm submission
      setDisabled(false);
      // Reset contact htmlForm fields after submission
      reset();
    }
  };

  return (
    <section className="bg-blue-50 dark:bg-slate-800 my-10" id="contact">
              ...
              <form id="contacthtmlForm">
                <div className="mb-6">
                  ...
                  <div className="mx-0 mb-1 sm:mb-4">
                    ...
                    <textarea id="textarea" name="message" cols={30} rows={5} placeholder="Write your message..." className="mb-2 w-full rounded-md border border-gray-400 py-2 pl-2 pr-4 shadow-md dark:text-gray-300 sm:mb-0"></textarea>
                  </div>
                </div>
                <div className="text-center">
                  <Button type="submit" variant={'greenBtn'} className="w-full bg-blue-800 text-white px-6 py-3 font-xl rounded-md sm:mb-0">Send Message</Button>
                </div>
              </form>
            </div>
          </div>
